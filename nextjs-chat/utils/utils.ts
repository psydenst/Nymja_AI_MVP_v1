...
/**
* Streams the bot response via SSE using fetch-event-source.
*/
  ...
  try {
    await fetchEventSource(url, {
      ...
      onmessage(event) {
        ...
        if (data.status === "thinking" && handlers.onThinking) {
          handlers.onThinking();
        }
        if (data.chunk) {
        ...
        if (data.complete && data.message) {
          handlers.onComplete(data.message);
        }
        if (data.error && handlers.onError) {
